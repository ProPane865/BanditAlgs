# Contextual Linear UCB Algorithm
...
for i in range(1, trials + 1):
    ...
    alpha = numpy.log(n_params * trials)

    # Calculated using covariance as inverse of Gramian
    variance = numpy.array([numpy.matmul(features[j].reshape(1, n_params), numpy.matmul(numpy.linalg.inv(gramian), features[j].reshape(n_params, 1))) for j in range(n_arms)]).reshape(1, n_arms)
    print(numpy.linalg.norm(variance))
    ucb = numpy.inner(params_estimate, features) + alpha * numpy.sqrt(variance)
    max_ind = numpy.argmax(ucb)
